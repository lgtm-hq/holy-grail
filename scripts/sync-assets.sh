...
sync_theme_css() {
	...
	local count
	count=$(find public/packages/css/dist/themes -maxdepth 1 -type f -name '*.css' | wc -l | tr -d ' ')
	echo "[sync-assets] Synced $count theme CSS files to public/"
}
...
all)
	...
	;;
*)
	echo "Usage: $0 {pagefind|themes|all}" >&2
	exit 1
	;;
esac

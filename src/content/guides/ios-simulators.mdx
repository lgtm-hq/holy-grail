...
> device (e.g., camera, GPS, Bluetooth). Use them for functional and UI automation
> testing with tools like <LinkBadge href="/guides/appium/" />, and validate on
> physical devices for performance-critical testing.
...
```bash
# List available runtimes for download
...
Starting with Xcode 16, you can use the `simctl` runtime commands:
...
```bash
# List installed runtimes and their sizes
xcrun simctl list runtimes
